...
use self::model_discovery::{
    DiscoveryEnd, ResolvedModel, ResolvedSemanticModels, discover_model, discover_semantic_models,
};
pub(crate) use self::run_execution::purpose_has_executable_supervisor;
#[cfg(test)]
use self::run_execution::{
    SupervisorRunRoute, supervise_run, supervise_run_with_registry, supervisor_run_route,
};
use self::runtime_dispatch::supervisor_loop;
...
use birdcode_backends::{
    BackendError, BackendErrorKind, BackendId, BackendInstanceIdentity, ModelBackend,
    ModelLoadState, StructuredInferenceRequest, StructuredInferenceResponse,
};
...
use birdcode_store::{DeadlineAppendOutcome, PlannerV2NotDispatchedReason, Store, StoreError};
...
mod tests {
    use super::model_discovery::{
        ResolvedSemanticModel, parse_reasoning, resolve_catalog, resolved_total_token_budget,
    };
    use super::*;
    use birdcode_backends::{
        ...
        InferenceEvidence, LoadedInstance, ModelCapabilities, ModelCatalog, ModelDescriptor,
        ModelId, ModelKind, NativeDiscoveryEvidence, NativeMatch, ReasoningSetting,
    };
    use birdcode_prompting::{
